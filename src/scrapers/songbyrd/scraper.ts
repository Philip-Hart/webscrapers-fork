...
async function scrapeEvent(
  ...
  const detailsContainer = await page.waitForSelector(".wpem-event-details") ?? "";
  ...
  const eventDetails = (
    (await page.evaluate(
      (detailsContainer) => detailsContainer.textContent,
      detailsContainer,
    )) ?? ""
  ).trim();

  // null if price string is empty
  const [ticketPrice, doorPrice] = parseTicketPrice(eventDetails);
